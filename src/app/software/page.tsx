...
interface SoftwareItem { id: string; name: string; category: string | null; minRequirements: string | null; recRequirements: string | null; size: string | null; image: string | null; active: boolean; }
...
export default function SoftwarePage() {
  ...
  const loadItems = async () => {
    try { const res = await fetch("/api/software"); if (res.ok) setItems(await res.json()); } catch {}
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    if (savedForm) {
      try {
        ...
        setMinReq(data.minReq || ""); setRecReq(data.recReq || ""); setSize(data.size || "");
  ...
  const saveCategories = (cats: string[]) => { setCategories(cats); localStorage.setItem("softwareCategories", JSON.stringify(cats)); };
  ...
  const resetForm = () => { setName(""); setCategory(""); setMinReq(""); setRecReq(""); setSize(""); setImageUrls([]); setImagePreviews([]); setEditingId(null); setShowForm(false); };
  ...
  const handleTakePhoto = () => {
    sessionStorage.setItem("softwareFormData", JSON.stringify({ editingId, name, category, minReq, recReq, size, imageUrls, imagePreviews }));
  ...
  const saveItem = async () => {
    ...
    try {
      if (editingId) {
        const res = await fetch("/api/software", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ id: editingId, name, category, minRequirements: minReq, recRequirements: recReq, size, image: imageData }) });
        if (res.ok) { showToast("✅ Actualizado"); resetForm(); loadItems(); }
      } else {
        const res = await fetch("/api/software", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name, category, minRequirements: minReq, recRequirements: recReq, size, image: imageData }) });
        if (res.ok) { showToast("✅ Agregado"); resetForm(); loadItems(); }
      }
  ...
  const editItem = (item: SoftwareItem) => {
    ...
    setMinReq(item.minRequirements || ""); setRecReq(item.recRequirements || ""); setSize(item.size || "");
  ...
  const deleteItem = async (id: string) => {
    ...
    const token = localStorage.getItem("token"); if (!token) return;
    try { const res = await fetch("/api/software", { method: "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ id }) }); if (res.ok) { showToast("🗑️ Eliminado"); loadItems(); } } catch {}
  };
    ...
    <div className="main-content" style={{ minHeight: "100vh", background: "var(--bg-primary)", paddingLeft: 200, paddingTop: 0 }}>
      ...
      <aside className={`sidebar-desktop${menuOpen ? " open" : ""}`} style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 200, transition: "transform 0.3s ease", background: "rgba(12,12,18,0.95)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", zIndex: 45, padding: "0 10px" }}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflow: "auto", padding: "4px 0" }}>
          {[{ label: "Panel Principal", path: "/dashboard", icon: "📋", r: "all" }, { label: "Servicios", path: "/services", icon: "🛠️", r: "admin" }, { label: "Inventario", path: "/inventory", icon: "📦", r: "admin" }, { label: "Software", path: "/software", icon: "🎮", r: "admin", active: true }, { label: "Mensajes", path: "/messages", icon: "💬", r: "all" }, { label: "Escáner", path: "/scanner", icon: "📷", r: "all" }, { label: "Cotizaciones", path: "/quotations", icon: "🧾", r: "all" }, { label: "Extracto", path: "/extracto", icon: "📊", r: "admin" }].filter((item: any) => item.r === "all" || user?.role === "admin").map((item) => (
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 150, padding: 20 }}>
            <div style={{ width: "100%", maxWidth: 600, maxHeight: "90vh", overflow: "auto", background: "var(--bg-card)", borderRadius: 20, border: "1px solid rgba(139,92,246,0.2)", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", animation: "fadeScale 0.3s ease-out" }}>
              ...
              <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ padding: "12px 16px", background: "rgba(59,130,246,0.04)", borderRadius: 12, border: "1px solid rgba(59,130,246,0.08)" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#3b82f6", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>⚙️ Requisitos Mínimos</div>
                  <textarea value={minReq} onChange={(e) => setMinReq(e.target.value)} placeholder="CPU: Intel i3&#10;RAM: 4 GB&#10;GPU: Intel HD&#10;Almacenamiento: 20 GB" rows={4} style={{ ...fieldStyle, resize: "vertical", fontSize: 11 }} />
                </div>

                <div style={{ padding: "12px 16px", background: "rgba(16,185,129,0.04)", borderRadius: 12, border: "1px solid rgba(16,185,129,0.08)" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#10b981", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>🚀 Requisitos Recomendados</div>
                  <textarea value={recReq} onChange={(e) => setRecReq(e.target.value)} placeholder="CPU: Intel i7&#10;RAM: 16 GB&#10;GPU: RTX 3060&#10;Almacenamiento: 50 GB SSD" rows={4} style={{ ...fieldStyle, resize: "vertical", fontSize: 11 }} />
                </div>
              ...
              return (
                <div key={item.id} onClick={() => setExpandedId(isExpanded ? null : item.id)} style={{ background: "var(--bg-card)", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden", animation: `fadeIn 0.3s ease-out ${i * 0.04}s both`, position: "relative", cursor: "pointer" }}>
                  ...
                  <div style={{ padding: "14px 16px" }}>
                    ...
                    {isExpanded && (item.minRequirements || item.recRequirements) && (
                      <div style={{ marginBottom: 10, animation: "fadeIn 0.2s ease-out" }}>
                        {item.minRequirements && (
                          <div style={{ padding: "8px 10px", background: "rgba(59,130,246,0.04)", borderRadius: 8, borderLeft: "3px solid #3b82f6", marginBottom: 6 }}>
                            <div style={{ fontSize: 9, color: "#3b82f6", fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>⚙️ Req. Mínimos</div>
                            <div style={{ fontSize: 10, color: "var(--text-secondary)", lineHeight: 1.5, whiteSpace: "pre-line" }}>{item.minRequirements}</div>
                          </div>
                        )}
                        {item.recRequirements && (
                          <div style={{ padding: "8px 10px", background: "rgba(16,185,129,0.04)", borderRadius: 8, borderLeft: "3px solid #10b981" }}>
                            <div style={{ fontSize: 9, color: "#10b981", fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>🚀 Req. Recomendados</div>
                            <div style={{ fontSize: 10, color: "var(--text-secondary)", lineHeight: 1.5, whiteSpace: "pre-line" }}>{item.recRequirements}</div>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {!isExpanded && (item.minRequirements || item.recRequirements) && (
                      <div style={{ fontSize: 10, color: "#8b5cf6", marginBottom: 8, fontWeight: 600 }}>▾ Ver requisitos</div>
                    )}
