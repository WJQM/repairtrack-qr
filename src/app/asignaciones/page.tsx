...
export default function AsignacionesPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)", paddingLeft: 200 }}>
      ...
      {/* SIDEBAR */}
      <aside style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 200, background: "rgba(12,12,18,.95)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", zIndex: 45, padding: "0 10px" }}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflow: "auto", padding: "4px 0" }}>
          {[{ l: "Mis Asignaciones", p: "/asignaciones", i: "📋", on: true }, { l: "Mensajes", p: "/messages", i: "💬" }, { l: "Escáner", p: "/scanner", i: "📷" }, { l: "Cotizaciones", p: "/quotations", i: "🧾" }].map(x => (
            <button key={x.p} className={`sb${(x as any).on ? " on" : ""}`} onClick={() => router.push(x.p)}>
