                  ...
                  <div key={repair.id} onClick={() => setExpandedId(isExpanded ? null : repair.id)} style={{ background: "var(--bg-card)", borderRadius: 16, border: `1px solid ${isExpanded ? st.color + "30" : "var(--border)"}`, cursor: "pointer", transition: "all 0.25s", animation: `fadeIn 0.3s ease-out ${i * 0.04}s both`, overflow: "hidden" }}>
                    {!isExpanded && (
                      <div className="card-compact" style={{ display: "flex", alignItems: "stretch" }}>
                        {firstImage ? (<div className="card-img" onClick={(e) => { e.stopPropagation(); setViewImage(firstImage); }} style={{ width: 120, minHeight: 120, flexShrink: 0, cursor: "pointer", position: "relative" }}><img src={firstImage} alt={repair.device} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />{repairImages.length > 1 && <div style={{ position: "absolute", bottom: 4, right: 4, background: "rgba(0,0,0,0.7)", borderRadius: 6, padding: "2px 6px", fontSize: 10, color: "#fff", fontWeight: 700 }}>+{repairImages.length - 1}</div>}</div>) : (<div className="card-img" style={{ width: 120, minHeight: 120, flexShrink: 0, background: st.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 38, borderRight: "1px solid var(--border)" }}>{st.icon}</div>)}
                        <div className="card-info" style={{ flex: 1, padding: "12px 16px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 6, minWidth: 0 }}><div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><span style={{ fontFamily: "monospace", fontSize: 11, color: "#6366f1", fontWeight: 700, background: "rgba(99,102,241,0.08)", padding: "2px 7px", borderRadius: 5 }}>{repair.code}</span><span style={{ fontSize: 13, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{[repair.device, repair.brand, repair.model].filter(Boolean).join(" ")}</span></div><div className="filter-btns" style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{repair.clientName && <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.1)", color: "var(--text-secondary)" }}>👤 {repair.clientName}</span>}{repair.clientPhone && <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.1)", color: "var(--text-muted)" }}>📱 {repair.clientPhone}</span>}{repair.technician && <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, background: "rgba(168,85,247,0.06)", border: "1px solid rgba(168,85,247,0.1)", color: "#a855f7" }}>🔧 {repair.technician.name}</span>}</div><p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>🔧 {repair.issue}</p></div>
                        <div className="card-status" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", flexShrink: 0 }}><div style={{ textAlign: "right" }}><span style={{ padding: "4px 10px", borderRadius: 14, fontSize: 10, fontWeight: 600, color: st.color, background: st.bg, border: `1px solid ${st.color}20`, display: "inline-block", whiteSpace: "nowrap" }}>{st.icon} {st.label}</span><div style={{ fontSize: 9, color: "var(--text-muted)", marginTop: 4, textAlign: "center" }}>Bs. {repair.estimatedCost}</div></div><span style={{ fontSize: 12, color: "var(--text-muted)" }}>▾</span></div>
                      </div>
                    )}
                    {isExpanded && (
