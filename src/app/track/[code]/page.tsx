...
function parseNotesData(notesField: string | null): { notes: string; services: string[]; software: string[] } {
  if (!notesField) return { notes: "", services: [], software: [] };
  ...
  const notesParts = parts.filter(p => !p.startsWith("Servicios: ") && !p.startsWith("Software: "));
  ...
  return { notes: notesParts.join(" | "), services, software };
}
...
export default function TrackPage() {
  ...
  const handleClose = () => { if (window.history.length > 1) window.history.back(); else window.close(); };
  ...
  const { notes, services, software } = parseNotesData(repair.notes);
    ...
    <div style={{ minHeight: "100vh", background: "#050507", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 500, position: "relative", zIndex: 1, background: "linear-gradient(180deg, rgba(17,17,24,0.95), rgba(8,8,12,0.98))", borderRadius: 28, border: `1px solid ${status.color}15`, boxShadow: `0 0 80px ${status.color}06, 0 25px 60px rgba(0,0,0,0.5)`, overflow: "hidden", opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(24px)", transition: "all 0.7s cubic-bezier(0.16, 1, 0.3, 1)" }}>
        ...
        <div style={{ padding: "20px 28px" }}>
          ...
          {/* Botón cerrar */}
          <button onClick={handleClose} style={{ width: "100%", padding: "14px", background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 14, color: "#818cf8", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            ← Cerrar Seguimiento
          </button>
        </div>
